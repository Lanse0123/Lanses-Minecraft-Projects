...
public class CaveSpiderBoss extends CaveSpiderEntity {

    public CaveSpiderBoss(World world) {
        super(EntityType.CAVE_SPIDER, world);

        this.setCustomName(Text.literal("CaveSpiderBoss").formatted(Formatting.YELLOW));
        this.setCustomNameVisible(true);

        ItemStack helmet = new ItemStack(Items.LEATHER_HELMET);
        helmet.addEnchantment(Enchantments.PROTECTION, 9);
        ...
        chestplate.addEnchantment(Enchantments.PROTECTION, 9);
        ...
        leggings.addEnchantment(Enchantments.PROTECTION, 9);
        ...
        boots.addEnchantment(Enchantments.PROTECTION, 9);
    ...
    public static void Neutralize(ServerWorld world, LivingEntity boss) {
    ...
    public static void attack(ServerWorld world, LivingEntity boss) {
        ...
        nearbyPlayers = world.getEntitiesByClass(PlayerEntity.class, new Box(pos.subtract(5, 5, 5), pos.add(5, 5, 5)), e -> true);
        if (!nearbyPlayers.isEmpty()){
            for (PlayerEntity player : nearbyPlayers){
                player.addStatusEffect(new StatusEffectInstance(StatusEffects.BLINDNESS, 50, 1));
                player.addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 50, 1));
            }
        }
        ...
        if (random.nextInt(75) == 25) {
            ...
            switch (attackIndex) {
                ...
                case 2 -> AttackPoisonLaser(world, boss);
            }
        }
    }
            ...
            for (int x = -radius; x <= radius; x++) {
                for (int y = -radius; y <= radius; y++) {
                    for (int z = -radius; z <= radius; z++) {
                        ...
                        // Check if the block is air and replace it with cobweb
    ...
    private static void AttackSummonMinions(ServerWorld world, LivingEntity boss) {
        ...
        if (!nearbyPlayers.isEmpty()){
            ...
            int spawnCount = random1.nextInt(6) + 3;
    ...
    public static void spawnReactor(Entity entity, ServerWorld world) {
        CaveSpiderBoss caveSpider = new CaveSpiderBoss(world);
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        ...
        dispatcher.register(literal("SummonBossCaveSpider")
                .requires(source -> source.hasPermissionLevel(2)).executes(context -> {
                    ServerPlayerEntity player = context.getSource().getPlayer();
                    if (player != null) {
                        World world = player.getEntityWorld();
                        CaveSpiderBoss caveSpider = new CaveSpiderBoss(world);
                        caveSpider.refreshPositionAndAngles((player.getX() + 10), player.getY(), player.getZ(), player.getYaw(), player.getPitch());
                        world.spawnEntity(caveSpider);
                        context.getSource().sendFeedback(() -> Text.literal("Cave Spider Boss has been summoned!"), true);
                        return 1;
                    } else {
                        context.getSource().sendError(Text.literal("Failed to summon Cave Spider Boss. Player not found."));
                        return 0;
                    }
                }));
    }
