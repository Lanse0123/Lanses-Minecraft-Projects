...
//TODO - change this to block box range instead of all blocks. This will save space drastically.
public class RegionStorage {
    ...
    public static Path getRegionFile(Path worldFolder, String dimensionId, int regionX, int regionZ) {
        Path regionDir = worldFolder.resolve("ImageRegion").resolve(dimensionId);
        try {
    ...
    public static void resetPermaSave(MinecraftServer server) {
        ...
        for (String dimFolder : dimensionFolders) {
            Path regionFolder = worldFolder.resolve("ImageRegion").resolve(dimFolder);
        ...
        ImageWorld.regionCache.clear();
        ImageWorld.permaSaveNeedsWrite = true;
    }
}